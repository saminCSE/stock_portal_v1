...
class TradingViewController extends Controller
{
        ...
        if ($q) {
            ...
            $collections = Instrument::select('id', 'instrument_code', 'name')
                ...
                ->Where(function ($query) use($q) {
                    $query->where('instrument_code', 'LIKE', '%' . $q . '%')
                    ->orWhere('name', 'LIKE', '%' . $q . '%');
    ...
    public function history(Request $request)
    {
        ...
        $collections = DataBanksEod::select('*', DB::raw('UNIX_TIMESTAMP(date) as unix_lm_date_time'))
            ->selectRaw("CASE WHEN close = 0 THEN low ELSE close  END AS close")
            ->where('date', '>=', $from_c)
            ->where('date', '<=', $to_c)
        ...
        if ($collections->count()) {
            ...
            foreach ($collections as $key => $row) {
                ...
                // if($key == 0) {
                //     $response['TimeFrom'] = $row->unix_lm_date_time;
                // }
                // else if($total - 1 == $key) {
                //     $response['TimeTo'] = $row->unix_lm_date_time;
                // }
            }
            return response()->json($response);
        } else {

            $eod_data = DataBanksEod::getSingleHistoryData(['from_date' => $from_c, 'to_date' => $to_c, 'instrument_id' => $getSymbol->id]);
            if ($eod_data) {
                $response['Data'] = [];
                $response['nextTime'] = $eod_data->unix_lm_date_time;
            } else {

                $response['Data'] = [];
                $response['nextTime'] = false;
            }
    ...
    private function  totalDaycount($string)
    {
        switch ($string) {
            case "5D":
                return '- 5';
    ...
    public function prevIndexChart(Request $request)
    {
        ...
        $range_cal = $request->range_cal;

        $prev_day = $this->totalDaycount($range_number . '' . $range_cal);
        $from_c = changeDate($to_c, $prev_day);
    ...
    {
        //   DB::enableQueryLog();
        $collections = DB::table('markets')
            ->leftJoin('index_values', 'index_values.market_id', '=', 'markets.id')
            ->select('markets.trade_date', 'markets.trd_total_value as totalValueMN','markets.trd_total_volume as volume','markets.percentage_deviation as pdeviation','index_values.capital_value as indexValue')
            ->groupBy('markets.trade_date')
            ->orderBy('markets.trade_date', 'DESC')
    ...
    public function TopTrade(Request $request)
    {
        //   DB::enableQueryLog();
        $today = $request->tdate;
        $collections = DataBanksIntraday::with('instrument:id,instrument_code as name')
            ->select('id', 'instrument_id', 'trade_date', 'yday_close_price', 'total_trades')
            ->selectRaw("CASE WHEN pub_last_traded_price = 0 THEN spot_last_traded_price ELSE pub_last_traded_price END AS last_traded_price")
            ->selectRaw("CASE WHEN pub_last_traded_price = 0 THEN ((spot_last_traded_price - yday_close_price) * 100 ) / yday_close_price ELSE ((pub_last_traded_price - yday_close_price) * 100) / yday_close_price END AS pchange")
            ->where('trade_date', '=', $today)
            ->whereRaw("id IN (SELECT MAX(id) FROM data_banks_intradays WHERE trade_date='$today' GROUP BY (instrument_id))")
    ...
    public function symbolIndexChart(Request $request) {
        $instrument_id = $request->instrument_id;
        $last_id = $request->last_id;
        $from_c = $request->tdate;
        $to_c = $request->tdate;
