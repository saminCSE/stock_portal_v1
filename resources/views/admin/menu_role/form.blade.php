...
@section('content')
    <div class="card">
        <div class="card-body">
            @if(isset($item))
                {!! Form::model($item, ['route' => ["rolemenu.update"],'method' =>'PUT', 'class' => 'custom-validation', 'files' => true,'role'=>"form", 'id' => 'edit-form']) !!}
            @else
                {!! Form::open(['route' => ["rolemenu.store"], 'method'=>'POST', 'class' => 'custom-validation', 'files' => true, 'role'=>"form",'id' => 'add-form']) !!}
            @endif
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            {!! Form::select('role_id', $role, isset($item->role_id) ? $item->role_id : NULL, ['class' => 'form-control']) !!}
                ...
                @else --> --}}
                    @foreach($menu as $key=>$value)
                        <div class="col-md-12">
                            <div class="checkbox">
                                <label>
                                    <input type="checkbox" name="menu_id[{{$value->id}}][menu]" value="{{$value->id}}" /> {{ $value->menu_name }}
                        ...
                        @if(isset($value->submenu))
                            @foreach($value->submenu as $skey=>$svalue)
                                <div style="margin-left:30px;" class="col-md-10">
                                    <div class="checkbox">
                                        <label>
                                            <input type="checkbox" name="menu_id[{{$value->id}}][submenu][{{$svalue->id}}]" value="{{$svalue->id}}" /> {{ $svalue->menu_name }}
